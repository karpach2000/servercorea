...
*/
    ...
    fun addSession(sessionId: Long, sessionPas: Long) = addSession(CardsSession(sessionId, sessionPas))
 /*   override fun addSession(gamesSession: CardsSession): Boolean {
    ...
    fun stopCardsGame(sessionId: Long, sessionPas: Long): Boolean {
        logger.info("stopCardsGame($sessionId, $sessionPas)")
        if (isSessionExists(sessionId)) {
            val session = getSession(sessionId, sessionPas)
            session.stopCardsGame()
